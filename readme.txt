=== Simple Announcement With Exclusion ===
Contributors: 
Donate link: 
Tags: widget, category, hide, mini loop, shortcode
Requires at least: 3.3
Tested up to: 3.5.1
Stable tag: 1.4
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Wordpress plugin to set aside a category, and exclude it from the front page (display as widget or on a page or post with shortcode).

== Description ==

SAWE is a plugin to set aside a single category as an announcements section (or aside, or updates, or whatever) and display it via shortcode ( [sawe] ) or widget, with the option of also excluding posts from this category from showing up on the main feed.

== Installation ==

1. Upload /SAWE/ to your /wp-content/plugins directory.
2. Navigate to your plugins menu in your Wordpress admin.
3. Activate it.
4. You'll find settings for the plugin under Settings->SAWE.

Place the widget (optional) or use the shortcode [sawe] (optional).

== Frequently asked questions ==



== Screenshots ==

1. 
2. 

== Changelog ==


1.0 - 2/13/2013: initial release.
1.1 - 2/13/2013: cleaned up settings page.
1.2 - 2/13/2013: added [sawe] shortcode (to display SAWE on a post or a page without having to use the widget).
1.3 - 2/14/2013: added ability to show excerpt or full post content. Cleaned up code a bit.
1.4 - 2/15/2013: added default styles (that can be optionally loaded) - styled the options page, added relevant information.
If your theme doesn't support post thumbnails, SAWE will enable them.


== Upgrade notice ==

